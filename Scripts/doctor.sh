...
# The package floor and the supported CI lane come from Package.swift and the
# Linux workflow. The native macOS gate runs whatever Xcode the runner ships.
supported_swift_version="${required_swift_version:-6.4}"
swift_requirement_hint="Native macOS gates require Swift ${supported_swift_version}+ with SwiftPM and Foundation."
...
if [ "$host_os" = "Linux" ]; then
  ok "Pinned Linux image supplies Swift ${supported_swift_version} and Python 3"
fi
