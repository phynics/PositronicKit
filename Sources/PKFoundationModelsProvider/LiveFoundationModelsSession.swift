...
import PKUtilities
    ...
    /// the framework executes any registered `FoundationModels.Tool.call(arguments:)` itself while
    /// producing the response — tool calls/outputs are only observable after the fact via
    /// `session.transcript`,
