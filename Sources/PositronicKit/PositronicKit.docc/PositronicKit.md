...
- **Persistence layer**: Focused store protocols with one cohesive `ThreadRuntimeRepository` for
  Thread history and Turn transitions.
...
`ThreadHandle.startTurn(_:options:)` and `ThreadHandle.startDirectTurn(_:context:options:)` perform
all request and preparation work before returning an admitted `TurnHandle`:
...
- Managed execution captures the Agent attached to the Thread immediately before durable
  admission. Detached managed execution throws
  `TurnError.managedExecutionRequiresAttachedAgent`.
...
- After admission, `TurnHandle.events()` is nonthrowing. Provider and pipeline failures are
  delivered as terminal error events, while `outcome()` reads the durable terminal state and can
  throw when the caller cancels or a bounded wait expires.
