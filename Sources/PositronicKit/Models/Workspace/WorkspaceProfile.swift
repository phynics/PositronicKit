...
///   workspace record is persisted. `TimelineRecord.workingDirectory` stays `nil`. A minimal chat
///   runtime (one-shot `complete`/`stream`, or `createTimeline` with the default configuration)
///   has **no filesystem side effects**.
/// - `.ephemeralWorkspace`: PKRuntime owns a scratch directory under `root`. The directory
...
/// - `.hostManaged`: PKRuntime creates per-timeline directories under `root` (and seeds
...
public enum WorkspaceProfile: Sendable {
    ...
    /// creation, note seeding, and workspace-record persistence; `TimelineRecord.workingDirectory`
    /// is `nil`. This is the default.
    case noWorkspace

    /// PKRuntime owns an ephemeral per-timeline scratch directory under `root`.
    ///
    ...
    ///
    /// PKRuntime creates per-timeline directories under `root` (and seeds notes per
...
public extension WorkspaceProfile {
    ...
    /// root to anchor to. TimelineRecord creation itself is unaffected: `.noWorkspace` creates no
    ...
    /// Whether PKRuntime owns the lifecycle (creation + cleanup) of the per-timeline
