...
/// `(timelineID, sendID)`. Terminal paths remove the entry only when the sendID still matches
...
actor TimelineTaskRegistry {
    ...
    func register(_ task: Task<Void, Never>, sendID: UUID, for timelineID: UUID) {
        active[timelineID]?.task.cancel()
        active[timelineID] = ActiveSend(sendID: sendID, task: task)
    }
    ...
    /// one.
    @discardableResult
    func cancel(sendID: UUID, for timelineID: UUID) -> Bool {
        guard let current = active[timelineID], current.sendID == sendID else { return false }
        current.task.cancel()
        return true
    }
    ...
    /// entry.
    func removeIfActive(sendID: UUID, for timelineID: UUID) {
        guard let current = active[timelineID], current.sendID == sendID else { return }
