...
struct LLMStreamingStage: PipelineStage {
    ...
    func process(_ context: TurnContext) async throws -> AsyncThrowingStream<TurnEvent, Error> {
        ...
        // TurnEngine's entry point rejects turns that set both `structuredOutput` and
    ...
    private func streamResponse(
        ...
        for try await result in streamData {
            // Cooperative cancellation: throw out of the stage so `TurnEngine`'s catch path
    ...
    private func routeSidecarOutput(
        ...
        case let .completed(results):
            ...
            guard context.sidecarCommitPolicy == .everyModelRound else { return }
    ...
    private func handleStreamUsage(_ result: LLMStreamChunk, context: TurnContext) async {
