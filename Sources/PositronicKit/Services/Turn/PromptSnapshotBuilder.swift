...
///
/// Extracted from `TurnEngine` (PKARCH-001). The incremental-string assembly (PKR-10) is
            ...
            toolCalls: message.toolCalls?.compactMap { toolCall in
                ...
                do {
                    guard let parsed = try JSONSerialization.jsonObject(with: Data(toolCall.arguments.utf8)) as? [String: Any] else {
                        throw NSError(domain: "TurnEngine", code: -1, userInfo: [NSLocalizedDescriptionKey: "tool-call arguments are not a JSON object"])
                    }
                    arguments = parsed
                } catch {
