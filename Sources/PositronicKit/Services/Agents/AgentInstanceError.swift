...
public enum AgentInstanceError: PKError, Sendable {
    ...
    public var errorDescription: String? {
        ...
        case let .threadAgentMismatch(threadID, agentInstanceID, attachedAgentInstanceID):
            let attachment = attachedAgentInstanceID.map { "agent \($0) is attached" } ?? "no agent is attached"
            return "Agent instance \(agentInstanceID) is not attached to thread \(threadID); \(attachment)."
