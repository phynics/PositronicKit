...
public actor ThreadArchiver {
    ...
    public func archive(
        ...
        do {
            try await indexAndSaveMessages(
                messages,
                thread: archiveState.archivedThread,
                title: title
            )
        ...
        return archiveState.archivedThread.id
    }
    ...
    private struct ArchiveState {
        let archivedThread: Thread
        let previousThread: Thread?
    }

    private func resolveThread(threadID: UUID?, title: String) async throws -> ArchiveState {
        if let sid = threadID, let existing = try await persistence.fetchThread(id: sid) {
            ...
            return ArchiveState(archivedThread: updated, previousThread: existing)
        } else {
            var newThread = Thread(title: title)
            newThread.isArchived = true
            try await persistence.saveThread(newThread)
            return ArchiveState(archivedThread: newThread, previousThread: nil)
        }
    }

    private func rollback(_ state: ArchiveState, after originalError: Error) async {
        do {
            if let previousThread = state.previousThread {
                try await persistence.saveThread(previousThread)
            } else {
                try await persistence.deleteThread(id: state.archivedThread.id)
            }
        } catch {
            let operation = state.previousThread == nil ? "deleteThread" : "restoreThread"
            logger.error("""
            Archive rollback failed — thread: \(state.archivedThread.id.uuidString.prefix(8)), \
    ...
    private func indexAndSaveMessages(
        _ messages: [Message], thread: Thread, title: String
    ) async throws {
        for msg in messages {
