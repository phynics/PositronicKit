...
This guide describes the unreleased Next / v5 architecture on `main`. The
[stable `5.1.0` architecture](https://github.com/phynics/PositronicKit/blob/5.1.0/docs/Architecture.md)
...
| `ThreadPromptJournals` | `ThreadManager` and `TurnEngine`, for prompt cache state rather than Thread history |
...
stores. Use `PersistenceConfiguration.fullyPersistent(...)` when every store must survive a
process restart. The in-memory configuration implements the same contracts for tests and prototypes. The
...
| `AgentActivitySink` | Best-effort Agent lifecycle integration. It does not mutate Thread history |
...
The current boundary has no automatic semantic-memory retrieval stage. Hosts that already
have a retrieval system provide its bounded result through `AgentContextSource`. Per-Turn additions
use `TurnContextSource`, and model-directed note access uses Workspace file tools. The prompt layer's
