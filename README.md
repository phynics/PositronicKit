...
- Latest stable: `5.1.0`. Use the [immutable tagged documentation](https://github.com/phynics/PositronicKit/blob/5.1.0/README.md) and the semver dependency below for production.
...
pinned Linux development environment with Swift 6.3.3, and CI qualifies the Swift 6.4 lane.
...
```swift
.package(url: "https://github.com/phynics/PositronicKit.git", from: "5.1.0")
```

Public products follow semver. Continue with the tagged README for stable API examples. The
examples below describe the current Next public story and may advance beyond `5.1.0`.
...
> Release availability: this section documents the current Next channel. It is not part of the stable `5.1.0`
...
`LINUX_SWIFT_VERSION=6.4` to build the qualified Swift 6.4 variant instead:
...
make agent-verify LINUX_SWIFT_VERSION=6.4 # Run the same gate on Swift 6.4
```
