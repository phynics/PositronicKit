...
- **Precision-safe `AnyCodable` integers**: JSON integers now decode and encode through the
  ...
  but exhaustive switches over `AnyCodable` must handle the two new cases. Tool `Int` extraction
  accepts exact integral values and rejects fractional, out-of-range, or potentially lossy
