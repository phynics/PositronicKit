...
/// Coverage for `OpenAIClient` methods and the `PKOpenAI` factory that are not
...
"""#)
    ...
    // MARK: - PKTool-call recovery path
...
"""#
    ...
    func makeClientForOpenAI() {
        ...
        let client = PKOpenAI.makeClient(configuration: config)
