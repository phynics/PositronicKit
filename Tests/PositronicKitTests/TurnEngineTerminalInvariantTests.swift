...
/// error or cancellation), the runtime must not invoke `TurnPlugin.afterTurn`, build a
/// follow-up prompt snapshot, append messages, or start another LLM turn. These tests drive
/// the full `TurnEngine` turn loop (not a bare stage) so the outer-loop continuation decision
/// is exercised end to end.
@Suite(.serialized) @MainActor
struct TurnEngineTerminalInvariantTests {
    private let threadID = UUID()

    /// Mirrors `TurnEngineFailurePersistenceTests.withTurnEngineDependencies` but is kept local
    ...
    func noSecondLLMTurnOrInjectedMessageAfterTerminalFailure() async throws {
        ...
        try await withTurnEngineDependencies(plugins: [plugin]) { engine, mockLLM, mockPersistence in
...
private actor TerminalPluginRecorder: TurnPlugin {
