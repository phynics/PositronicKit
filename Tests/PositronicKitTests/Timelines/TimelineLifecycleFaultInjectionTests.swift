...
/// PKRR-007: TimelineRecord creation and workspace attachment must not leave partial state.
///
...
struct TimelineLifecycleFaultInjectionTests {
    ...
    func attachWorkspaceDoesNotResurrectPermanentlyDeletedTimeline() async throws {
        ...
        let manager = TimelineManager(
            stores: .init(
                ...
                runtimeRepository: timelineStore,
    ...
    func createTimelineHealthyLeavesNoOrphans() async throws {
        ...
        let workspaces = try await manager.getWorkspaces(for: timeline.id)
