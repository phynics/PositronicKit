...
struct ThreadSendToolTests {
    ...
    func enforcesRemoteDepthFromSourceHistory() async throws {
        ...
        let agentId = UUID()
        let source = Thread(workingDirectory: workspace.root.path, attachedAgentID: agentId)
        ...
        let destination = Thread(workingDirectory: workspace.root.path, attachedAgentID: agentId)
