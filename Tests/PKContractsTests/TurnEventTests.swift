...
@Suite final class TurnEventTests {
    @Test
    ...
    func testGenerationCancelledEvent() throws {
        ...
        let event = TurnEvent.generationCancelled()

        let encoder = JSONEncoder()
        let data = try encoder.encode(event)

        let decoder = JSONDecoder()
        let decoded = try decoder.decode(TurnEvent.self, from: data)
    ...
    func usageGuideEventPatternMatchesCurrentShape() {
        ...
        let toolErrorEvent = TurnEvent.error(.toolCallError(
            toolCallID: "call_1",
            name: "lookup_weather",
            error: "bad args"
        ))
        let completionEvent = TurnEvent.completion(.generationCompleted(
            message: Message(content: "done", role: .assistant),
            metadata: completionMeta
        ))
        let maxModelRoundsEvent = TurnEvent.maxModelRoundsReached()
        let deferredEvent = TurnEvent.deferredForExternalTool()
        ...
        if case .completion(.maxModelRoundsReached) = maxModelRoundsEvent {
            // success
        } else {
            Issue.record("Expected completion.maxModelRoundsReached event, got \(maxModelRoundsEvent)")
        }
    ...
    func toolCallIdentifierCompatibility() throws {
        ...
        for event in canonicalEvents {
            ...
            let decoded = try JSONDecoder().decode(TurnEvent.self, from: encoded)
            switch decoded {
    ...
    func completedEmptyEventRoundTripsThroughCodable() throws {
        let event = TurnEvent.completedEmpty(finishReason: "stop")
        let encoder = JSONEncoder()
        let decoder = JSONDecoder()
    ...
    func errorFromPlainErrorCarriesNilIdentity() throws {
        ...
        let event = TurnEvent.error(ProviderError())
    ...
    func errorFromStringCarriesNilIdentity() throws {
        let event = TurnEvent.error("boom")
    ...
    func directlyConstructedIdentityDefaultsToNotBlocked() {
        ...
        let identity = TurnEvent.ErrorIdentity(domain: PKErrorDomain.tool, code: 210)
